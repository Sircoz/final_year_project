...
class InputTypedExpr : public ITypedExpr {
 public:
  ...
  std::string toSQLString() const override {
    return "";
  }
  ...
  std::shared_ptr<const ITypedExpr> copyWInputs(
 ...
 public:
  ...
  std::string toString() const override {
    ...
    return str;
      ...
      std::string toSQLString() const override {
    std::map<const std::string, const std::string> SQLstr = {
      {"eq", "="},
      {"neq", "!="},
      {"gte", ">="},
      {"lte", "<="},
    ...
    for (size_t i = 0; i < inputs().size(); ++i) {
      ...
      if (i != 0) {
        str += " ";
        if(SQLstr.find(name()) == SQLstr.end()) {
          str += name();
        } else {
          str += SQLstr[name()];
        }
...
class LambdaTypedExpr : public ITypedExpr {
 public:
  ...
  std::shared_ptr<const ITypedExpr> copyWInputs(
      const std::vector<std::shared_ptr<const ITypedExpr>> inputs)
      const {
    return std::make_shared<LambdaTypedExpr>(signature_, body_);
  }
