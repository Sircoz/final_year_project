 ...
 public:
  ...
  void runFilterThenProject() { 
    for(int i = 0; i < queries.size(); i++){
        ...
        //run plan
        auto res = AssertQueryBuilder(execPlan).maxDrivers(1).queryCtx(queryCtx_).returnResults(pool());
        if(res == NULL) {
          res = makeRowVector({"id"}, {makeFlatVector<int32_t>({0})});
        }else {
          res = makeRowVector({queries[i][1]}, {res->childAt(idIndex)});
        }
    }
  }
  
  void runOne(){
    runFilterThenProject();
  }
