...
/*The plan is (Values->Filter->Project->ArrayAggregation->CrossJoin(queryId))* Queries -> LocalPartition. Each Filter node contains the filter for each query..*/
  ...
  void compilePlan(){
    ...
    plan = PlanBuilder(planNodeIdGenerator).localPartition({}, planList).planNode();
  }
...
void ProjectPlanArchitectureExperiment::compile() {
  compilePlan();
}
...
int main(int argc, char** argv) {
  __itt_domain* domainexec = __itt_domain_create("My Domain Execution"); 
  __itt_domain* domaincomp= __itt_domain_create("My Domain Compilation"); 
  __itt_string_handle* compilation = __itt_string_handle_create("Compolation");
  __itt_string_handle* execution = __itt_string_handle_create("Execution");
  ...
  demo.setup();
  __itt_task_begin(domaincomp, __itt_null, __itt_null, compilation);
  demo.compile();
  __itt_task_end(domaincomp);

  __itt_task_begin(domainexec, __itt_null, __itt_null, execution);
  demo.run();
  __itt_task_end(domainexec);
  return 0;
}