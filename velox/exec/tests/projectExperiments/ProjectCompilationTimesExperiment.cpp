...
class ProjectCompilationTimesExperiment : public VectorTestBase {
 public:
  ...
  ProjectCompilationTimesExperiment() {
  ...
  //Experiment
  void compareCompilationTimesExperiment(){
    std::cout << "Pick return rate: ";
    std::cin >> nbQueries;
    queriesLocation = "adult/queries_1000.txt";
    loadData();

    std::vector<double> compilationTimesToITypedExpr;
    std::vector<double> compilationTimesToExprSet;
    ...
    for(int i = 0; i < queries.size(); i++){
      ...
      auto beforecompilationToITypedExpr = std::chrono::high_resolution_clock::now();
      core::TypedExprPtr tpyedExpr = parseExpression(filter, asRowType(data->type()));

      auto aftercompilationToITypedExpr = std::chrono::high_resolution_clock::now();
      std::vector<core::TypedExprPtr> exprs = {tpyedExpr};
      std::make_unique<exec::ExprSet>(std::move(exprs), execCtx_.get());

      auto aftercompilationToExprSet = std::chrono::high_resolution_clock::now();

      std::chrono::duration<double, std::milli> compilationTimeToITypedExpr =
                aftercompilationToITypedExpr - beforecompilationToITypedExpr;
      std::chrono::duration<double, std::milli> compilationTimeToExprSet =
                aftercompilationToExprSet - aftercompilationToITypedExpr;

      compilationTimesToITypedExpr.push_back(compilationTimeToITypedExpr.count());
      compilationTimesToExprSet.push_back(compilationTimeToExprSet.count());
    }

    auto compilationITypedExpr = makeFlatVector<double>(compilationTimesToITypedExpr);
    auto compilationExprSet = makeFlatVector<double>(compilationTimesToExprSet);
    auto times = makeRowVector({"ityped", "exprset"},
                              {compilationITypedExpr, compilationExprSet});
    auto plan = PlanBuilder().values({times})
      .singleAggregation(
        {},
        {"avg(ityped) AS ityped_avg",
        "stddev(ityped) AS ityped_std",
        "avg(exprset) AS exprset_avg",
        "stddev(exprset) AS exprset_std"}).planNode();
...
void ProjectCompilationTimesExperiment::run() {
  compareCompilationTimesExperiment();
}

int main(int argc, char** argv) {
  folly::init(&argc, &argv, false);

  ProjectCompilationTimesExperiment demo;
  demo.run();
}
