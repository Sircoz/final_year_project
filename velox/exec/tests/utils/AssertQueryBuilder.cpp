...
std::pair<std::shared_ptr<Task>, std::vector<RowVectorPtr>> AssertQueryBuilder::assertAndReturnResults(
    const RowVectorPtr& expected) {
  auto [cursor, results] = readCursor();

  return std::make_pair(cursor->task(), results);
}

std::vector<RowVectorPtr> AssertQueryBuilder::returnRes() {
  auto [cursor, results] = readCursor();
...
RowVectorPtr AssertQueryBuilder::returnResults(memory::MemoryPool* pool) {
  auto [cursor, results] = readCursor();
...
AssertQueryBuilder::readCursor() {
  ...
  return test::readCursor(params_, [&](Task* task) {
    ...
    for (auto& [nodeId, nodeSplits] : splits_) {
      //std::cout << "Num drivers:" << task->driverCount() << std::endl;
