...
class AssertQueryBuilder {
 public:
  ...
  std::pair<std::shared_ptr<Task>, std::vector<RowVectorPtr>> assertAndReturnResults(const RowVectorPtr& expected);
  ...
  std::vector<RowVectorPtr> returnRes();
  ...
  RowVectorPtr returnResults(memory::MemoryPool* FOLLY_NONNULL pool);
 ...
 private:
  ...
  std::unique_ptr<folly::Executor> executor_{
      new folly::CPUThreadPoolExecutor(4)};
