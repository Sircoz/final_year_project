...
PlanBuilder& PlanBuilder::filterFromExpr(const core::TypedExprPtr& filterExpr) {
...
std::pair<PlanBuilder, core::TypedExprPtr> PlanBuilder::filterCustom(const std::string& filter) {
  core::TypedExprPtr expr = parseExpr(filter, planNode_->outputType(), options_, pool_);
  planNode_ = std::make_shared<core::FilterNode>(
      nextPlanNodeId(),
      expr,
      planNode_);
  PlanBuilder& ret = *this;
  return std::make_pair(ret, expr);
}
